 ...
 *
 * `cam: true` marks PIR-camera detectors, which additionally get a camera image
 * (snapshot on alarm). Every battery detector also gets `alarm_battery` (low-battery).
 */
function zoneProfile(detectorType) {
  const t = String(detectorType || '').toLowerCase();
  const base = ['measure_temperature', 'measure_battery', 'alarm_battery', 'alarm_tamper'];
  ...
  if (t.includes('temp') || t.includes('heat') || t.includes('pdht') || t.includes('pdtph'))
    return { icon: 'heat', cls: 'sensor', caps: ['measure_temperature', 'measure_battery', 'alarm_battery', 'alarm_tamper', 'alarm_heat'] };
  if (t.includes('panic') || t.includes('emergency') || t.includes('button') || t.includes('pdeb'))
    return { icon: 'panic', cls: 'button', caps: ['alarm_generic', 'measure_battery', 'alarm_battery', 'alarm_tamper'] };
  if (t.includes('cam')) // PIR-camera detectors: motion + camera snapshot on alarm
    return { icon: 'camera', cls: 'sensor', cam: true, caps: ['alarm_motion', ...base] };
  if (t.includes('pir') || t.includes('motion') || t.includes('curtain')
...
const PERIPHERALS = [
  { list: 'KeypadList',    item: 'Keypad',    type: 'keypad',    icon: 'keypad',   cls: 'sensor', caps: ['measure_temperature', 'measure_battery', 'alarm_battery', 'alarm_tamper'] },
  { list: 'SirenList',     item: 'Siren',     type: 'siren',     icon: 'siren',    cls: 'sensor', caps: ['measure_battery', 'alarm_battery', 'alarm_tamper'] },
  { list: 'RepeaterList',  item: 'Repeater',  type: 'repeater',  icon: 'repeater', cls: 'sensor', caps: ['measure_battery', 'alarm_battery', 'alarm_tamper'] },
  { list: 'CardReaderList', item: 'CardReader', type: 'cardreader', icon: 'generic', cls: 'sensor', caps: ['measure_battery', 'alarm_battery', 'alarm_tamper'] },
...
// Low-battery threshold (percent) for alarm_battery.
const LOW_BATTERY = 20;

module.exports = { zoneProfile, PERIPHERALS, LOW_BATTERY };
