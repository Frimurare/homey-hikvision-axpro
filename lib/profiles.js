 ...
 * (snapshot on alarm). Battery detectors report `measure_battery` (percentage); Homey warns on low battery automatically.
 */
function zoneProfile(detectorType) {
  ...
  if (t.includes('magnet') || t.includes('contact') || t.includes('shock') || t.includes('vibrat')) {
    // Combined magnet + shock detectors: the magnet half drives alarm_contact
    // (magnetOpenStatus); a shock trigger only shows in the zone `alarm` flag,
    // so those get alarm_generic as well — otherwise the shock is discarded.
    const shock = t.includes('shock') || t.includes('vibrat');
    return { icon: 'contact', cls: 'sensor', caps: shock ? ['alarm_contact', 'alarm_generic', ...base] : ['alarm_contact', ...base] };
  }
  if (t.includes('smoke'))
